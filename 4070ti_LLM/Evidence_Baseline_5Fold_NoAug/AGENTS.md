...
- `src/data.py` assembles augmented datasets and deterministic 5-fold splits; extend data utilities here.
- `src/model.py` houses the DeBERTa-v3 backbone, multi-layer MLP head, and loss implementations; create new architectures in sibling modules.
...
- Validation and test splits for each fold are sampled only from `is_augmented == False` rows; augmentation data feeds the training split exclusively.
