...
class CriteriaClassifier(nn.Module):
    ...
    def __init__(self, config: CriteriaModelConfig):
        ...
        self.pooling = str(config.pooling).lower()
        hidden = self.base_config.hidden_size
        layers: List[nn.Module] = []
        for size in config.hidden_sizes:
            layers.extend([nn.Linear(hidden, size), self._make_activation(), nn.Dropout(config.dropout)])
            hidden = size
    ...
    def _make_activation(self) -> nn.Module:
        """Return the configured activation layer."""
        name = str(self.config.activation).lower()
        if name == "relu":
            return nn.ReLU()
        if name == "gelu":
            return nn.GELU()
    ...
    def _apply_pooling(self, hidden_states: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        """Aggregate sequence representations according to the configured pooling strategy."""
        if self.pooling == "max":
            mask = attention_mask.unsqueeze(-1).to(dtype=torch.bool)
            masked = hidden_states.masked_fill(~mask, float("-inf"))
            values = masked.amax(dim=1)
            values = torch.where(torch.isfinite(values), values, hidden_states[:, 0])
            return values
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor, labels: Optional[torch.Tensor] = None):
        ...
        pooled = self._apply_pooling(outputs.last_hidden_state, attention_mask)
