...
def optimize_hardware_settings():
    ...
    if torch.cuda.is_available():
        ...
        print(f"🚀 Hardware optimizations applied for {device_name}")
        print(f"🔧 CUDA version: {cuda_version}")
        print(f"💾 Available memory: {total_memory:.1f} GB")
        print(f"🧠 CPU threads: 8 (optimized for i7-8700)")
        print(f"⚡ TF32 enabled: True (Ampere optimization)")
        print(f"🏃 cuDNN benchmark: True")

        # Check for BF16 support
        if hasattr(torch.cuda, 'is_bf16_supported') and torch.cuda.is_bf16_supported():
            print(f"🎯 BFloat16 support: Available (recommended for RTX 3090)")
        else:
            print(f"🎯 BFloat16 support: Not available, using Float16")
