...
def load_evidence_annotations(
    ...
    # Create symptom to criterion mapping (for backwards compatibility with criterion IDs)
    ...
    for (post_id, symptom), group in grouped:
        ...
        # Check if symptom is in criteria_map (use symptom name directly, not criterion ID)
        if symptom not in criteria_map:
            continue

        criterion_text = criteria_map[symptom]
        criterion_id = symptom_to_criterion.get(symptom, symptom)  # Use symptom as ID if not mapped
