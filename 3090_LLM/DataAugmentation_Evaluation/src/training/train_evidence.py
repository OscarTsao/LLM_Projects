...
def create_evidence_datasets(cfg: DictConfig) -> Tuple[EvidenceDataset, EvidenceDataset, EvidenceDataset]:
    ...
    examples = load_evidence_annotations(
        posts_path=cfg.training_mode.data.posts_path,
        annotations_path=cfg.training_mode.data.annotations_path,
        criteria_path=cfg.training_mode.data.get("criteria_path")
    )
...
def main(cfg: DictConfig) -> float:
    ...
    # Log output directory for easy access to results
    from hydra.core.hydra_config import HydraConfig
    hydra_cfg = HydraConfig.get()
    output_dir = hydra_cfg.runtime.output_dir
    logger.info(f"Results will be saved to: {output_dir}")
    logger.info(f"Model checkpoint will be saved to: {output_dir}/best/model.pt")

    # Initialize MLflow
    mlflow_utils.setup_mlflow(experiment_name="redsm5-evidence")
    ...
    try:
        with mlflow_run:
            ...
            # Use Hydra's actual output directory instead of the config value
            from hydra.core.hydra_config import HydraConfig
            hydra_cfg = HydraConfig.get()
            actual_output_dir = hydra_cfg.runtime.output_dir
            ...
            train_config = {
                ...
                "use_amp": cfg.model.get("use_amp", True),
                "use_bfloat16": cfg.model.get("use_bfloat16", True),  # Use bfloat16 by default to avoid overflow
                "early_stopping_patience": cfg.training_mode.get("early_stopping_patience", 10),
                "metric_for_best_model": cfg.training_mode.get("metric_for_best_model", "span_f1"),
...
def train_evidence_agent(
    ...
    # Training setup
    # Note: bfloat16 doesn't need GradScaler (no overflow/underflow issues)
    # Only use GradScaler for fp16, not for bfloat16
    use_bfloat16 = config.get("use_bfloat16", False)
    scaler = None if use_bfloat16 else (torch.amp.GradScaler('cuda') if config["use_amp"] else None)
    best_metric = 0.0
    patience_counter = 0
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for step, batch in enumerate(progress_bar):
            ...
            # Forward pass
            if config["use_amp"]:
                # Use bfloat16 for mixed precision to avoid overflow issues
                dtype = torch.bfloat16 if config.get("use_bfloat16", False) else torch.float16
