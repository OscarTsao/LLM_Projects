...
LORA_TARGET_FALLBACKS: Tuple[Tuple[str, ...], ...] = (
    ("query", "key", "value"),
    ("q_proj", "k_proj", "v_proj"),
    ("query_proj", "key_proj", "value_proj"),
    ("in_proj",),  # DeBERTa uses a single projection layer
)

LORA_DEFAULT_TARGETS: Dict[str, Tuple[str, ...]] = {
    "bert": ("query", "key", "value"),
    "roberta": ("query", "key", "value"),
    "deberta": ("in_proj",),  # DeBERTa uses in_proj for Q, K, V
    "deberta-v2": ("in_proj",),
    "deberta-v3": ("in_proj",),
    "electra": ("query_proj", "key_proj", "value_proj"),
    "xlnet": ("q", "k", "v"),
}
...
class BaseClassificationHead(nn.Module):
    ...
    def __init__(self, input_dim: int, cfg: DictConfig) -> None:
        ...
        self.feedforward, self.output_dim = _build_feedforward(
            input_dim, hidden_dims, activation, dropout
        )
...
class MultiLabelClassificationHead(BaseClassificationHead):
    ...
    def __init__(self, input_dim: int, cfg: DictConfig) -> None:
        ...
        classifier_dropout = cfg.get(
            "classifier_dropout", cfg.get("layers", {}).get("dropout", 0.1)
        )
...
class EvidenceModel(nn.Module):
    ...
    def _maybe_apply_lora(self, encoder_cfg: DictConfig) -> None:
        ...
        def has_all_targets(model: nn.Module, targets: Iterable[str]) -> bool:
        ...
        def get_encoder_type() -> str:
            """Determine encoder type from model config or encoder type setting."""
            # First try to get from the actual model config
            encoder_model_type = getattr(getattr(self.encoder, "config", None), "model_type", "")
            if encoder_model_type:
                return encoder_model_type.lower()

            # Fallback to encoder config type
            return encoder_cfg.get("type", "").lower()

        encoder_type = get_encoder_type()

        # Get target modules from config first, then defaults
        cfg_targets = lora_cfg.get("target_modules")
        if cfg_targets:
            target_modules = tuple(cfg_targets)
        else:
            # Use encoder type to get appropriate defaults
            default_targets = LORA_DEFAULT_TARGETS.get(encoder_type, ())
            target_modules = default_targets if default_targets else LORA_TARGET_FALLBACKS[0]

        # Build candidate sets for fallback
        candidate_sets: List[Tuple[str, ...]] = [target_modules]
        # Add all fallback options if not already included
        for fallback in LORA_TARGET_FALLBACKS:
            if fallback not in candidate_sets:
                candidate_sets.append(fallback)

        selected_targets: Optional[Tuple[str, ...]] = None
        for candidate in candidate_sets:
            if has_all_targets(self.encoder, candidate):
                selected_targets = candidate
                break

        if selected_targets is None:
            # List available attention modules for debugging
            attention_modules = [
                name
                for name, _ in self.encoder.named_modules()
                if any(target in name for target in ["query", "key", "value", "proj", "attention"])
            ]
            warnings.warn(
                f"LoRA configuration requested but no matching attention projection modules were found "
                f"for encoder type '{encoder_type}'. Available attention modules: {attention_modules[:10]}... "
                f"Skipping LoRA injection for this encoder.",
                stacklevel=2,
            )
            return

        # Use FEATURE_EXTRACTION to avoid PEFT adding 'labels' parameter expectation
        # (we handle classification with custom heads)
        config = LoraConfig(
            ...
            target_modules=list(selected_targets),
        )
    ...
    def forward(
        ...
        # Build encoder inputs (only include token_type_ids if provided)
        encoder_kwargs = {
            "input_ids": input_ids,
