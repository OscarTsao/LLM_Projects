...
def train_loop(cfg: DictConfig) -> Dict[str, float]:
    ...
    retain_local_checkpoint = bool(
        OmegaConf.select(cfg, "training.checkpoint.keep_local", default=True)
    )
    log_checkpoint_to_mlflow = bool(
        OmegaConf.select(cfg, "training.checkpoint.log_to_mlflow", default=True)
    )
    ...
    for epoch in epoch_progress:
        ...
        if improved:
            ...
            if log_checkpoint_to_mlflow and mlflow.active_run():
    ...
    if not retain_local_checkpoint and best_checkpoint.exists():
        best_checkpoint.unlink()
        try:
            next(checkpoint_dir.iterdir())
