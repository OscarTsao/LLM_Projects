...
def ensure_output_dir(cfg: DictConfig) -> Path:
    # Try to get Hydra's runtime output directory first
    try:
    ...
    # Fallback: check if hydra config is in cfg
    if 'hydra' in cfg and 'run' in cfg.hydra and 'dir' in cfg.hydra.run:
        return Path(cfg.hydra.run.dir)

    # Final fallback: generate timestamped directory
    base_dir = cfg.get('output_dir', 'outputs/training')
