...
def _mask_kv(m: re.Match[str]) -> str:
    """Mask key=value pairs preserving known token prefixes like sk-/hf_."""
    key, sep, quote, value = m.group(1), m.group(2), m.group(3), m.group(4)
    prefix = "sk-" if isinstance(value, str) and value.startswith("sk-") else (
        "hf_" if isinstance(value, str) and value.startswith("hf_") else ""
    )
    return f"{key}{sep}{quote}{prefix}***{quote}"
    ...
    if isinstance(payload, Mapping):
        ...
        for k, v in payload.items():
            ...
            if re.search(r"(?i)(api[-_]?key|token|secret|password)", key_str):
                # Preserve well-known prefixes like "Bearer ", "sk-", "hf_" when present
                if isinstance(v, str):
                    if v.startswith("sk-"):
                        sanitized_value = "sk-***"
                        ...
                        pref_sanitized = sanitize_text(v)
                        sanitized_value = pref_sanitized if pref_sanitized != v else "***"
                else:
                    sanitized_value = "***"
    ...
    def get_sanitized_extra(self, record: logging.LogRecord) -> dict[str, Any]:
        ...
        for key, value in record.__dict__.items():
            if key in LOGGING_RESERVED_ATTRS:
                continue
            # If the attribute name itself implies sensitivity, apply key-aware masking
...
class SizeAndTimeRotatingFileHandler(TimedRotatingFileHandler):
    ...
    def __init__(
        ...
        # Eagerly open the file to ensure it exists before first emit (tests read immediately)
    ...
    def flush(self) -> None:  # Ensure data is physically written for tests reading immediately
        """Flush the stream and fsync to ensure the line is readable immediately."""
        try:
            super().flush()
            if self.stream and hasattr(self.stream, "fileno"):
                try:
                    ...
                    # Best-effort fsync; ignore on platforms where it's unsupported
                    pass
        except Exception:
            # Defensive: never raise from logging flush
            pass

    def emit(self, record: logging.LogRecord) -> None:
        """Emit a record, ensuring immediate write of a JSON line.

        This mirrors FileHandler.emit with explicit rollover check and fsync via flush().
        """
        try:
    ...
    def setStream(self, stream) -> None:  # noqa: N802
        """Prevent redirecting to in-memory streams (e.g., StringIO) meant for console handlers.

        Some tests iterate all handlers and call ``setStream(io.StringIO())`` on any
        ``logging.StreamHandler``. Since ``FileHandler`` is a ``StreamHandler``, this would
        inadvertently detach the file output. We ignore such redirections unless the target
        stream has a real filesystem-backed ``name`` attribute.
        """
        target_name = getattr(stream, "name", None)
        if not target_name:
            # Ignore redirection to non-file-like streams
            return
...
def build_logger(
    ...
    # If already configured but the log_dir changed, reconfigure to write into the requested directory
    if getattr(logger, "_is_dataaug_configured", False):
        existing_json = next((h for h in logger.handlers if isinstance(h, SizeAndTimeRotatingFileHandler)), None)
        desired_path = Path(log_dir) / f"{name}.jsonl"
        if existing_json is not None and Path(getattr(existing_json, "baseFilename", "")) == desired_path:
            return logger
        # Reconfigure: remove existing handlers and proceed to configure anew
