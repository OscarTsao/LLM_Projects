...
def _prepare_dataloaders(
    ...
    # Parse augmentation methods from comma-separated string
    aug_methods_str = aug_cfg.get("aug.methods", "") if aug_cfg else ""
    aug_methods = [m.strip() for m in aug_methods_str.split(",") if m.strip()] if aug_methods_str else []
    ...
    # Disable multiprocessing when using CUDA to avoid pickling issues
    # and improve GPU utilization (data loading is not the bottleneck with GPU)
    use_workers = 0 if torch.cuda.is_available() else hardware_cfg.num_workers
