        ...
        try:
            ...
            if isinstance(data, dict):
                if 'criteria' in data:
                    ...
                    for crit_id, crit_text in data['criteria'].items():
...
def load_redsm5_data(csv_path: str, criteria_dict: Dict[str, str]) -> pd.DataFrame:
    ...
    column_mappings = {
        ...
        'criterion_id': ['criterion_id', 'criterionId', 'criterion'],
        'label': ['label', 'is_evidence', 'evidence']
    }
    ...
    required = ['post_id', 'sentence', 'criterion_id', 'label']
    ...
    # Remove rows with missing criteria
    missing_criteria = df['criterion'].isna().sum()
    if missing_criteria > 0:
        logger.warning(f"Removing {missing_criteria} rows with missing criteria")
