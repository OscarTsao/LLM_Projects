...
from psy_agents_noaug.utils.storage import ensure_directory, prune_directory_except
...
def create_dataloaders(
    ...
    dataloader_kwargs = get_optimal_dataloader_kwargs(
        ...
        prefetch_factor=cfg.dataset.get("prefetch_factor"),
    )
...
def main(cfg: DictConfig):
    ...
    if cfg.training.get("tf32_enabled", False) and torch.cuda.is_available():
    ...
    if cfg.training.get("compile_model", False) and hasattr(torch, "compile"):
        ...
        try:
            ...
            print(
                f"torch.compile failed ({compile_error}); continuing without compilation."
            )
    ...
    print(f"Mixed precision: {cfg.training.get('amp_dtype', 'float16')}")
    print(f"TF32 enabled: {cfg.training.get('tf32_enabled', False)}")
    print(f"torch.compile: {cfg.training.get('compile_model', False)}")
    ...
    trainer = Trainer(
        ...
        amp_dtype=cfg.training.get("amp_dtype", "float16"),
    ...
    # Archive best artifacts for long-term storage
    artifact_base = ensure_directory(Path("artifacts") / "models" / cfg.project)
    run_identifier = run_id or Path(cfg.hydra.run.dir).name
    archive_dir = ensure_directory(artifact_base / run_identifier)

    if best_checkpoint_path.exists():
        copy2(best_checkpoint_path, archive_dir / "best_checkpoint.pt")

    latest_checkpoint = output_dir / "latest_checkpoint.pt"
    if latest_checkpoint.exists():
        copy2(latest_checkpoint, archive_dir / "latest_checkpoint.pt")
    ...
    summary_payload = {
        ...
        "best_validation_metric": final_metrics.get(
            f"best_{cfg.training.monitor_metric}", None
        ),
        "test_metrics": test_metrics,
    }
    with (archive_dir / "summary.json").open("w", encoding="utf-8") as fp:
        json.dump(summary_payload, fp, indent=2)

    OmegaConf.save(cfg, str(archive_dir / "config.yaml"))

    # Keep checkpoints required for resume only
    prune_directory_except(
        output_dir,
        keep_filenames={"best_checkpoint.pt", "latest_checkpoint.pt"},
    )
