...
from psy_agents_noaug.architectures.share.models.model import Model
...
def _evaluate(
    ...
    model.eval()
    losses: list[float] = []
    crit_logits_all: list[torch.Tensor] = []
    crit_labels_all: list[torch.Tensor] = []
    ...
    with torch.no_grad():
        for batch in dataloader:
            inputs = {
                ...
                if k not in ("labels", "start_positions", "end_positions")
            }
            labels = batch["labels"].to(device)
            start_positions = batch["start_positions"].to(device)
            end_positions = batch["end_positions"].to(device)

            criteria_logits, start_logits, end_logits = model(**inputs)
            cls_loss = cls_loss_fn(criteria_logits, labels)
            span_loss = (
                span_loss_fn(start_logits, start_positions)
                + span_loss_fn(end_logits, end_positions)
            ) / 2
            loss = (
                loss_weights.get("criteria", 1.0) * cls_loss
                + loss_weights.get("evidence", 1.0) * span_loss
            )
            losses.append(loss.item())

            crit_logits_all.append(criteria_logits.detach().cpu())
            crit_labels_all.append(labels.detach().cpu())
    ...
    crit_logits_cat = torch.cat(crit_logits_all)
    crit_labels_cat = torch.cat(crit_labels_all)
    span_start_logits_cat = torch.cat(span_start_logits_all)
    span_end_logits_cat = torch.cat(span_end_logits_all)
    span_start_labels_cat = torch.cat(span_start_labels_all)
    span_end_labels_cat = torch.cat(span_end_labels_all)

    cls_metrics = _classification_metrics(crit_logits_cat, crit_labels_cat)
    span_metrics = _span_metrics(
        span_start_logits_cat,
        span_end_logits_cat,
        span_start_labels_cat,
        span_end_labels_cat,
    )
...
def train(
    ...
    *,
    ...
    if resume and training_state_exists("share"):
        logger.info("Resuming training from last checkpoint.")
        checkpoint = load_training_state(
            "share", model=model, optimizer=optimizer, scheduler=scheduler
        )
    ...
    with (
        mlflow_run(name=config.get("project", "share")) if mlflow_cfg else nullcontext()
    ):
        if mlflow_cfg:
            import mlflow
        ...
        for epoch in range(start_epoch, epochs):
            ...
            train_cls_metrics = _classification_metrics(
                torch.cat(epoch_cls_logits), torch.cat(epoch_cls_labels)
            )
            train_span_metrics = _span_metrics(
                torch.cat(epoch_start_logits),
                torch.cat(epoch_end_logits),
                torch.cat(epoch_start_labels),
                torch.cat(epoch_end_labels),
            )
            train_joint = (train_cls_metrics["f1"] + train_span_metrics["span_em"]) / 2
            train_metrics = {
                ...
                "joint_score": train_joint,
            }

            val_metrics = _evaluate(
                model, val_loader, device, cls_loss_fn, span_loss_fn, loss_weights
            )
            ...
            metric_value = prefixed_val.get(
                monitor_metric, val_metrics.get("joint_score")
            )
    ...
    if test_loader is not None:
        ...
        try:
            ...
            test_metrics = _evaluate(
                best_model, test_loader, device, cls_loss_fn, span_loss_fn, loss_weights
            )
