...
conda create -n criteria-baseline python=3.10 -y
conda activate criteria-baseline
...
Change the number of folds by overriding `dataset.cross_validation.num_folds` when launching `train.py`.

To re-score a saved checkpoint without training:

```bash
make evaluate                             # uses outputs/trial_0043_rebuild/fold_1/best_model.pt
# evaluate a different fold
python evaluate.py evaluation.fold_index=3 evaluation.checkpoint=outputs/trial_0043_rebuild/fold_3/best_model.pt
```
