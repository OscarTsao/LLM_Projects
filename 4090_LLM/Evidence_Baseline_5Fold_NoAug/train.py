...
def main(cfg: DictConfig) -> None:
    ...
    if best_epoch_mean is not None:
        mean_str = (
            f"{best_epoch_mean:.2f}"
            if isinstance(best_epoch_mean, (int, float)) and math.isfinite(best_epoch_mean)
            else str(best_epoch_mean)
        )
        std_str = (
            f"{best_epoch_std:.2f}"
            if isinstance(best_epoch_std, (int, float)) and math.isfinite(best_epoch_std)
            else str(best_epoch_std)
        )
