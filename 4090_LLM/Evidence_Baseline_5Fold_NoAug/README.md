...
- `src/data.py` – lightweight dataset assembly (ground truth + hybrid/nlpaug/textattack augmentation) and deterministic 5-fold group splits.
- `src/model.py` – DeBERTa encoder with multi-layer MLP head plus BCE loss tuned for the evidence head.
...
- Backbone `microsoft/deberta-v3-base` with sequence length 256 and three-layer MLP head `[768, 384, 256]`.
- Binary cross-entropy loss with gradient checkpointing enabled to mirror the span-F1 ≈ 0.81 trial (augmentation disabled).
- Optimizer AdamW with learning rate `5.72e-6`, weight decay `9.64e-3`, warmup ratio `0.166`, gradient accumulation `x2`, and 20 training epochs.
