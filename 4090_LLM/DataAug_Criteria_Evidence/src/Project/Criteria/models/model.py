...
class Model(torch.nn.Module):
    ...
    def forward(
        ...
        # DistilBERT and RoBERTa don't support token_type_ids
        model_type = getattr(self.encoder.config, 'model_type', '').lower()
        supports_token_type_ids = model_type not in ['distilbert', 'roberta', 'xlm-roberta']

        if supports_token_type_ids and token_type_ids is not None:
