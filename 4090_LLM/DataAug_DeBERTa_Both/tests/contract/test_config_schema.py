...
class TestConfigSchema:
    ...
    def test_data_config_has_required_fields(self):
        ...
        assert "local_data" in config
        assert "splits" in config
        assert "batch_size" in config
        assert "max_length" in config
        
        # Verify dataset ID is correct
        assert config["dataset_id"] == "local/redsm5"
        
        # Verify local dataset configuration
        local_data = config["local_data"]
        assert "posts_file" in local_data
        assert "annotations_file" in local_data
        assert "groundtruth_file" in local_data
        assert "split_seed" in local_data
        assert "split_ratios" in local_data
