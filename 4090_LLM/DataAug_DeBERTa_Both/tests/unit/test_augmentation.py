...
class TestTextAugmenter:
    ...
    def test_augment_batch_preserves_originals(self):
        ...
        augmented = augmenter.augment_batch(examples, split="train")
    ...
    def test_augment_batch_skips_validation(self):
        """Validation split should never be augmented."""
        config = AugmentationConfig(probability=1.0)
        augmenter = TextAugmenter(config)

        examples = [
            {"text": "Validation example 1"},
            {"text": "Validation example 2"},
        ]

        augmented = augmenter.augment_batch(examples, split="validation")

        # Augmented output must match originals exactly
        assert augmented == examples
