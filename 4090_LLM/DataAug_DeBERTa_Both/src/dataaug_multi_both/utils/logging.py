...
def _mask_secret_match(match: re.Match[str]) -> str:
    masked_value = _mask_secret_value(match.group(4))
    return f"{match.group(1)}{match.group(2)}{match.group(3)}{masked_value}{match.group(3)}"

SANITIZATION_PATTERNS: tuple[tuple[re.Pattern[str], Callable[[re.Match[str]], str] | str], ...] = (
...
def sanitize_log_payload(payload: Any) -> Any:
        ...
        for k, v in payload.items():
            ...
            if re.search(r"(?i)(api[-_]?key|token|secret|password)", key_str):
                if isinstance(sanitized_value, str):
                    sanitized_dict[key_str] = _mask_secret_value(sanitized_value)
                else:
    ...
    if isinstance(payload, (list, tuple, set, frozenset)):
        ...
        if isinstance(payload, set):
            return {item for item in sanitized_items}
...
class JsonLogFormatter(SanitizingFormatter):
    ...
    def _safe_get(self, record: logging.LogRecord, attr: str) -> Optional[str]:
        value = getattr(record, attr, None)
        if value is None:
            return None
        return str(sanitize_log_payload(value))

    def _build_extra(self, record: logging.LogRecord) -> Optional[Dict[str, Any]]:
        ...
        if extra:
            ...
            extra = cast(Dict[str, Any], sanitize_log_payload(extra))
...
def build_logger(
    ...
    if getattr(logger, "_is_dataaug_configured", False):
        existing_paths = {
            ...
            if isinstance(handler, SizeAndTimeRotatingFileHandler)
        }
        if str(json_path) in existing_paths:
            return logger
        logger._is_dataaug_configured = False  # type: ignore[attr-defined]
...
def format_storage_exhaustion_error(
    ...
    retention_policy_hint: Optional[str] = None,
) -> str:
    """
    ...
    lines.append(f"    2. Remove stale checkpoints: find {trial_dir} -name 'checkpoint_*' -mtime +3 -delete")
    if retention_policy_hint:
        lines.append(
            f"    3. Adjust retention policy: hydra override 'trainer.checkpoint.retention={retention_policy_hint}'"
        )
    else:
        lines.append("    3. Reduce retention in Hydra config (trainer.checkpoint.retention=<smaller value>)")
    lines.append("    4. Re-run the trial after cleanup: poetry run python -m dataaug_multi_both.cli.resume")
