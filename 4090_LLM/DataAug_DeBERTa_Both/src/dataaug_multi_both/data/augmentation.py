...
class AugmentationConfig:
    ...
    def __post_init__(self):
        ...
        valid_probability = 0.0 <= self.probability <= 0.5 or self.probability == 1.0
        if not valid_probability:
            raise ValueError(
                f"Augmentation probability must be in [0.0, 0.5] or exactly 1.0, got {self.probability}"
            )
...
class TextAugmenter:
    ...
    def augment_batch(
        ...
        Args:
            ...
            split: Dataset split identifier; augmentation runs only for "train"
        ...
        if resolved_split != "train":
            logger.debug("Skipping augmentation for '%s' split.", resolved_split)
