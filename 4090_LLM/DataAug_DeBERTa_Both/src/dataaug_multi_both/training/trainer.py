...
def seed_everything(base_seed: int) -> Dict[str, int]:
    ...
    if torch is not None:
        ...
        try:
            ...
            LOGGER.debug("Failed to enable deterministic algorithms; training may be non-deterministic.")
...
def configure_hardware() -> None:
    """Configure CUDA/CuDNN settings with reproducibility guarantees."""
    ...
    if hasattr(torch.backends, "cuda"):
        try:
            matmul = torch.backends.cuda.matmul  # type: ignore[attr-defined]
            if hasattr(matmul, "allow_tf32"):
                matmul.allow_tf32 = False  # type: ignore[assignment]
            if hasattr(matmul, "allow_fp16_reduced_precision_reduction"):
                matmul.allow_fp16_reduced_precision_reduction = False  # type: ignore[assignment]
            if hasattr(matmul, "allow_bf16_reduced_precision_reduction"):
                matmul.allow_bf16_reduced_precision_reduction = False  # type: ignore[assignment]
        except (AttributeError, RuntimeError):
            LOGGER.debug("Unable to configure CUDA matmul determinism.", exc_info=True)
    ...
    if hasattr(torch.backends, "cudnn"):
        try:
            ...
            LOGGER.debug("Unable to configure CuDNN determinism.", exc_info=True)

    if hasattr(torch, "set_float32_matmul_precision"):
        try:
            torch.set_float32_matmul_precision("highest")
        except RuntimeError:
            LOGGER.debug("Unable to set float32 matmul precision to 'highest'.", exc_info=True)
...
def _to_numpy(array: object, dtype: object = np.float64) -> np.ndarray:
    if torch is not None and isinstance(array, torch.Tensor):
        return array.detach().cpu().numpy().astype(dtype, copy=False)
...
class Trainer:
    ...
    def __init__(
        ...
        self._cached_state: Optional[TrainingState] = None
    ...
    def resume(self) -> TrainingState:
        ...
        with self._resume_lock:
            ...
            try:
                ...
                self.logger.info("No checkpoints found for trial %s; starting fresh.", self.config.trial_id)
    ...
    def tune_validation_threshold(
        ...
        *,
        ...
        """Tune decision thresholds for logits against ground-truth labels.

        Returns
        -------
        Dict[str, object]
            Dictionary containing the macro-F1 score, the selected thresholds, and the thresholding mode.
        """

        logits_arr = _to_numpy(logits)
        labels_arr = _to_numpy(labels, dtype=np.int8)
        score, thresholds = tune_threshold(logits_arr, labels_arr, per_class=per_class)
        return {
            "macro_f1": float(score),
            "thresholds": thresholds,
            "threshold_mode": "per_class" if per_class else "global",
        }
