...
make train-new        # fresh run (alias: make train)
make train-resume     # resume from outputs/trial_0043_rebuild/state_latest.pt
...
Artifacts land under `outputs/trial_0043_rebuild/` by default:

- `best_model.pt` – best validation checkpoint (macro-F1 monitored)
...
```bash
make evaluate                             # uses outputs/trial_0043_rebuild/best_model.pt
