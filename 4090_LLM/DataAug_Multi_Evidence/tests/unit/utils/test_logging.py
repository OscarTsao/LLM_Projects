...
@pytest.mark.unit
def test_json_logger_masks_values(tmp_path: Path) -> None:
    logger_name = "test_logger"
    logger = build_logger(logger_name, log_dir=tmp_path)
...
@pytest.mark.unit
def test_format_storage_exhaustion_error_lists_artifacts(tmp_path: Path) -> None:
    message = format_storage_exhaustion_error(
        ...
        trial_dir=tmp_path,
        artifact_sizes=[
            (tmp_path / "checkpoint_epoch10.pt", 8.2),
            (tmp_path / "checkpoint_epoch09.pt", 7.8),
        ],
