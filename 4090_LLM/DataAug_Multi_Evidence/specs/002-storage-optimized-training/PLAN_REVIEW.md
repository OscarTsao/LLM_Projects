...
✅ **III. Single-Agent Architecture**: Evidence binding only; metrics limited to evidence extraction
...
```markdown
- **A-009**: Primary data source is Hugging Face datasets hub (dataset ID: irlab-udc/redsm5, revision: main). If hub is unavailable, system may fall back to local `Data/redsm5/*.csv` files. Both sources must produce identical train/val/test splits for reproducibility.
```
