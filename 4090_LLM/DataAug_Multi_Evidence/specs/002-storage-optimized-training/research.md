...
This document consolidates research findings and design decisions for the storage-optimized evidence-only NLP training pipeline with extensive hyperparameter optimization capabilities.
...
## 2. Evidence Binding Architecture

### Decision: Span Extraction Heads Only

**Rationale**:
- Repository migrated to single-agent evidence binding (v2.0)
- Focus HPO on span extraction head architecture and training strategies

**Architecture Components**:
- Shared transformer encoder (30-model catalog)
- Evidence binding head options: start/end linear, start/end MLP, biaffine, BIO+CRF, sentence reranker
...
- Compute metrics:
  - **Evidence binding**: Exact match, span F1 (token-level overlap), precision, recall, character-level F1
...
2. **Evidence Head Architecture** (span_head choices)
