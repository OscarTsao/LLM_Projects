...
- External data lives in `../gemini_reranker/baselines/dataaug_trial_0043/`; treat it as read-only.
...
Configuration derives from the Hydra stack in `config/`; update the relevant group (e.g., `training/baseline.yaml`) and document overrides in commits. Use Hydra CLI overrides (`python train.py training.learning_rate=3e-5`) for experiments rather than editing defaults. Coordinate schema or directory changes with teams consuming `../gemini_reranker/baselines/dataaug_trial_0043/`. Keep secrets out of configs; load credentials from environment variables or `.env` files ignored by Git.
