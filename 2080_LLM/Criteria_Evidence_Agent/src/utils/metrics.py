...
def compute_multi_label_metrics(
    ...
    try:
        metrics[f"val_{head_name}_roc_auc"] = roc_auc_score(
...
def compute_span_metrics(
    ...
    if np.any(valid_start & valid_end):
        joint_correct = (
            (start_preds == start_targets) &
            (end_preds == end_targets) &
            valid_start &
            valid_end
        )
        metrics[f"val_{head_name}_span_acc"] = float(
            joint_correct[valid_start & valid_end].mean()
        )
