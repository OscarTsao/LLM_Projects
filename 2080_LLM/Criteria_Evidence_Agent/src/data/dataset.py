...
class PostDataset(Dataset):
    ...
    def __init__(
        ...
        for _, row in df.iterrows():
            char_spans = None
            if evidence_char_spans_field and evidence_char_spans_field in row and not pd.isna(row[evidence_char_spans_field]):
                char_spans = _normalise_spans(_parse_json_field(row[evidence_char_spans_field]))
            self.char_spans.append(char_spans)

            token_labels = None
            if evidence_token_labels_field and evidence_token_labels_field in row and not pd.isna(row[evidence_token_labels_field]):
                token_labels = _normalise_labels(_parse_json_field(row[evidence_token_labels_field]))
...
class TokenizedDataCollator:
    ...
    def __init__(
        ...
        self.token_ignore_index = token_head_cfg.get("ignore_index", -100) if token_head_cfg else -100
    ...
    def __call__(self, batch: Sequence[Dict[str, object]]) -> Dict[str, torch.Tensor]:
        ...
        multi_labels = torch.tensor(np.stack([sample["multi_labels"] for sample in batch], axis=0), dtype=torch.float32)
        ...
        if self.token_head_enabled:
            token_labels = torch.full((batch_size, seq_len), self.token_ignore_index, dtype=torch.long)
        else:
            token_labels = None
        ...
        if self.need_offsets and offset_mapping is not None:
            for i in range(batch_size):
                ...
                if self.span_head_enabled:
                    start_idx, end_idx = self._span_positions_from_offsets(offsets, spans or [], self.span_ignore_index)
