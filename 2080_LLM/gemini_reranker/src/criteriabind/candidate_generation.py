    ...
    *,
    ...
    for sample in samples:
        ...
        for criterion in sample.criteria:
            criterion_text = criterion.display_text
            ...
            criterion_meta = (
                criteria_meta.get(criterion.id)
                if criterion.id in criteria_meta
                else criteria_meta.get(criterion_text, {})
            )
            positives = criterion.metadata.get("positives", criterion_meta.get("positives", []))
            has_recall = _positive_recall(candidates, positives)
            if positives:
                total_with_gold += 1
                hits += int(has_recall)
            criterion_hash = criterion.id or criterion_id(sample.id, criterion_text)
