...
def _serialize_samples(notes: Iterable[NoteExample], split: str) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    for note in notes:
        criteria_specs: list[dict[str, object]] = []
        criteria_details: dict[str, object] = {}
        for criterion in note.criteria:
            cid = criterion_id(note.note_id, criterion.text)
            criteria_specs.append(
                {
                    "id": cid,
                    "name": criterion.text,
                    "definition": criterion.text,
                    "metadata": {
...
def write_split(
    ...
    write_jsonl(root / f"{split}_samples.jsonl", _serialize_samples(notes_list, split))
