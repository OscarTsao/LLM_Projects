...
from ..metrics import compute_ranking_metrics, mean_average_precision
...
class Evaluator:
    ...
    def _evaluate_ranker(self, model, dataloader, device: str, amp_dtype: torch.dtype) -> dict[str, float]:
        ...
        total_pairs = 0
        correct_pairs = 0
        group_candidates: dict[str, dict[tuple[int, str], float]] = defaultdict(dict)
        with torch.no_grad():
            for batch in dataloader:
                ...
                pos_scores_np = pos_scores.detach().cpu().tolist()
                neg_scores_np = neg_scores.detach().cpu().tolist()
                for idx, group_id in enumerate(batch["group_ids"]):
                    pos_key = (1, batch["pos_text"][idx])
                    neg_key = (0, batch["neg_text"][idx])
                    group_candidates[group_id][pos_key] = max(
                        group_candidates[group_id].get(pos_key, float("-inf")),
                        pos_scores_np[idx],
                    )
                    group_candidates[group_id][neg_key] = max(
                        group_candidates[group_id].get(neg_key, float("-inf")),
                        neg_scores_np[idx],
                    )

        grouped_scores = {
            group_id: [(label, score) for (label, _), score in candidates.items()]
            for group_id, candidates in group_candidates.items()
        }
        map_at_10 = mean_average_precision(grouped_scores)
        ranking_metrics = compute_ranking_metrics(grouped_scores, k=10)
        pair_acc = correct_pairs / max(total_pairs, 1)
        model.train()
        return {
            "pair_accuracy": pair_acc,
            "map@10": map_at_10,
            "p@1": ranking_metrics.get("p@1", 0.0),
            "mrr": ranking_metrics.get("mrr", 0.0),
            "ndcg@10": ranking_metrics.get("ndcg@10", 0.0),
            "groups_evaluated": float(len(grouped_scores)),
        }
