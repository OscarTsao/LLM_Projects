...
from ..judge import GeminiJudge, GeminiMissingDependencyError, MockJudge
...
def _build_provider(app_cfg: AppConfig) -> tuple[Callable[[Iterable[JudgingJob]], list[Judgment]], str, str]:
    ...
    if provider == "gemini":
        ...
        if not api_key and not app_cfg.judge.vertex_enabled:
            if app_cfg.judge.fallback_to_mock:
                LOGGER.warning("Gemini API key missing; falling back to mock judge.")
                mock = MockJudge(
                    temperature=float(app_cfg.judge.temperature),
                    model_name="offline-mock-v1",
                )
                return mock.batch, "mock", mock.model_name
            raise RuntimeError(
                "judge.provider=gemini requires GEMINI_API_KEY (or judge.api_key) unless vertex_enabled=true."
            )
        try:
            judge = GeminiJudge(app_cfg, api_key=api_key)
        ...
        return judge.batch, provider, app_cfg.judge.model
