...
from ..data.real_loader import load_samples
...
def main(cfg: DictConfig) -> None:
    ...
    samples: list[Sample]
    if app_cfg.data.source == "jsonl":
        if not samples_path.exists():
            raise FileNotFoundError(samples_path)
        samples = [Sample.model_validate(row) for row in read_jsonl(samples_path)]
    else:
        samples = load_samples(app_cfg, split)
        if not samples:
            raise FileNotFoundError(f"No samples available for split '{split}'")
        samples_path.parent.mkdir(parents=True, exist_ok=True)
        write_jsonl(samples_path, [sample.to_dict() for sample in samples])
        LOGGER.info("Materialised %d samples to %s", len(samples), samples_path)
