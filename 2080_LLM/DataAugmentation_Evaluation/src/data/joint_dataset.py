...
def create_joint_dataset(
    ...
    for _, row in ground_truth_df.iterrows():
        post_id = row['post_id']
        post_text = row['post_text']  # Post text
        criterion_id = row['criterion']  # Criterion ID
        criteria_label = row['label']

        # Get criterion text from CRITERIA dictionary
        from .criteria_descriptions import CRITERIA
        criterion_text = CRITERIA.get(criterion_id, criterion_id)
