...
def load_posts_data():
    ...
    # Load negative posts
    negative_df = pd.read_csv('/experiment/YuNing/Psy_RAG/Data/mental_health_posts_negative_generated.csv')
    print(f"Loaded {len(negative_df)} negative posts")

    # Load positive posts
    positive_df = pd.read_csv('/experiment/YuNing/Psy_RAG/Data/translated_post_positive_random.csv')
    print(f"Loaded {len(positive_df)} positive posts")

    # Extract text from appropriate columns
    negative_posts = negative_df['post'].dropna().tolist()
