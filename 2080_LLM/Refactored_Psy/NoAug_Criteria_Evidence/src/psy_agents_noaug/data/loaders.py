...
def load_field_map(config_path: Union[str, Path]) -> Dict[str, Any]:
...
class ReDSM5DataLoader:
    ...
    def _validate_config(self):
        ...
        if self.data_source == 'local':
            ...
            raise ValueError(f"Invalid data_source: {self.data_source}. Must be 'local' or 'huggingface'")
    ...
    def load_annotations(self) -> pd.DataFrame:
        ...
        if self.data_source == 'local':
            ...
            if self.hf_cache_dir:
                dataset_kwargs["cache_dir"] = str(self.hf_cache_dir)
            dataset = load_dataset(self.hf_dataset_name, 'annotations', **dataset_kwargs)
            df = dataset['train'].to_pandas()
    ...
    def load_dsm_criteria(self, dsm_path: Union[str, Path]) -> List[Dict[str, str]]:
    ...
    def get_valid_criterion_ids(self, dsm_path: Union[str, Path]) -> Set[str]:
...
def load_splits_json(splits_path: Union[str, Path]) -> Tuple[List[str], List[str], List[str]]:
...
class DSMCriteriaLoader:
    ...
    def __init__(self, dsm_path: Union[str, Path]):
    ...
    def load_criteria(self) -> List[Dict[str, str]]:
        ...
        """
        with open(self.dsm_path, 'r', encoding='utf-8') as f:
            criteria = json.load(f)
