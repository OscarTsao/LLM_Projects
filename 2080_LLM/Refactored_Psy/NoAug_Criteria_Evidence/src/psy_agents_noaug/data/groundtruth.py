...
def normalize_status_value(value: Any, status_map: Dict[str, List]) -> Optional[int]:
...
def create_criteria_groundtruth(
    ...
    if valid_criterion_ids:
        ...
        if invalid_ids:
            if field_map.get('validation', {}).get('fail_on_invalid_criterion_id', True):
    ...
    if missing_posts:
        if field_map.get('validation', {}).get('fail_on_missing_post_id', True):
            raise ValueError(f"Post IDs in annotations not found in posts: {missing_posts}")
        else:
            print(f"WARNING: Dropping {len(missing_posts)} annotations with missing post_ids")
...
def create_evidence_groundtruth(
    ...
    df['evidence_text'] = df['cases_parsed'].apply(
        lambda x: x.get(cases_config.get('text', 'text'), '') if isinstance(x, dict) else ''
    )
    df['start_char'] = df['cases_parsed'].apply(
        lambda x: x.get(cases_config.get('start_char', 'start_char'), None) if isinstance(x, dict) else None
    )
    df['end_char'] = df['cases_parsed'].apply(
        lambda x: x.get(cases_config.get('end_char', 'end_char'), None) if isinstance(x, dict) else None
    )
    df['sentence_id'] = df['cases_parsed'].apply(
        lambda x: x.get(cases_config.get('sentence_id', 'sentence_id'), None) if isinstance(x, dict) else None
    )
    ...
    if missing_posts:
        if field_map.get('validation', {}).get('fail_on_missing_post_id', True):
            ...
            print(f"WARNING: Dropping annotations with missing post_ids")
            df = df[df['post_id'].isin(post_ids_set)].copy()
    ...
    return df[['post_id', 'criterion_id', 'case_id', 'evidence_text', 
               'start_char', 'end_char', 'sentence_id']]
...
def validate_strict_separation(
    ...
    assert 'evidence_text' in evidence_df.columns, "Evidence must have 'evidence_text' column"
    
    # Check that criteria does NOT have evidence-specific columns
    evidence_specific = {'evidence_text', 'case_id', 'start_char', 'end_char', 'sentence_id'}
    criteria_specific = set(criteria_df.columns) & evidence_specific
    assert not criteria_specific, (
...
class GroundTruthValidator:
    ...
    def validate_evidence_groundtruth(self, df: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        if 'evidence_text' in df.columns:
            empty_count = (df['evidence_text'].isna() | (df['evidence_text'] == '')).sum()
            if empty_count > 0:
