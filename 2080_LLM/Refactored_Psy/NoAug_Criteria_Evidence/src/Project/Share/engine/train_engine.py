...
def _evaluate(
    ...
    with torch.no_grad():
        for batch in dataloader:
            inputs = {k: v.to(device) for k, v in batch.items() if k not in ("labels", "start_positions", "end_positions")}
    ...
    span_metrics = _span_metrics(span_start_logits_cat, span_end_logits_cat, span_start_labels_cat, span_end_labels_cat)
...
def train(
    ...
    *,
    ...
    if resume and training_state_exists("share"):
        ...
        checkpoint = load_training_state("share", model=model, optimizer=optimizer, scheduler=scheduler)
    ...
    with mlflow_run(name=config.get("project", "share")) if mlflow_cfg else nullcontext():
        if mlflow_cfg:
            import mlflow
