...
from src.dataaug_multi_both.data.dataset import Dataset, DatasetDict, DatasetLoadError, DatasetValidator, load_hf_dataset
...
class TestDatasetValidator:
    ...
    def test_validate_dataset_id_valid(self):
        ...
        assert DatasetValidator.validate_dataset_id("redsm5")
        assert DatasetValidator.validate_dataset_id("irlab-udc/redsm5")
        assert DatasetValidator.validate_dataset_id("any_valid_string")
    ...
    def test_validate_splits_all_present(self):
        ...
        mock_dataset = DatasetDict({
            ...
            "validation": Dataset.from_list([{"text": "c"}]),
            "test": Dataset.from_list([{"text": "d"}])
        })
    ...
    def test_validate_split_disjointness_valid(self):
        ...
        mock_dataset = DatasetDict({
            "train": Dataset.from_list([{"post_id": 1}, {"post_id": 2}, {"post_id": 3}]),
            "validation": Dataset.from_list([{"post_id": 4}, {"post_id": 5}]),
            "test": Dataset.from_list([{"post_id": 6}, {"post_id": 7}])
        })
    ...
    def test_validate_split_disjointness_overlap(self):
        ...
        mock_dataset = DatasetDict({
            "train": Dataset.from_list([{"post_id": 1}, {"post_id": 2}, {"post_id": 3}]),
            "validation": Dataset.from_list([{"post_id": 3}, {"post_id": 4}]),  # 3 overlaps
            "test": Dataset.from_list([{"post_id": 5}, {"post_id": 6}])
        })
...
class TestLoadHFDataset:
    ...
    @patch('src.dataaug_multi_both.data.dataset._build_redsm5_from_local')
    def test_load_dataset_success(self, mock_build):
        """Test successful dataset loading."""
        mock_dataset = DatasetDict({
            "train": Dataset.from_list([{"post_id": "1", "post_text": "a", "criteria_labels": [0]*9},
                                       {"post_id": "2", "post_text": "b", "criteria_labels": [0]*9}]),
            "validation": Dataset.from_list([{"post_id": "3", "post_text": "c", "criteria_labels": [0]*9}]),
            "test": Dataset.from_list([{"post_id": "4", "post_text": "d", "criteria_labels": [0]*9}])
        })
        ...
        assert metadata["dataset_id"] == "redsm5"
        assert metadata["revision"] == "local"
        assert "resolved_hash" in metadata

    def test_load_dataset_invalid_id(self):
        """Test that unsupported dataset ID raises error."""
        with pytest.raises(DatasetLoadError, match="Unknown dataset"):
            load_hf_dataset("unsupported_dataset")

    @patch('src.dataaug_multi_both.data.dataset._build_redsm5_from_local')
    def test_load_dataset_missing_splits(self, mock_build):
        ...
        with pytest.raises(DatasetLoadError, match="Missing required splits"):
            load_hf_dataset("redsm5")

    @patch('src.dataaug_multi_both.data.dataset._build_redsm5_from_local')
    def test_load_dataset_file_error(self, mock_build):
        """Test that file errors produce actionable error."""
        mock_build.side_effect = DatasetLoadError("CSV files not found")

        with pytest.raises(DatasetLoadError, match="Failed to load dataset.*Remediation"):
            load_hf_dataset("redsm5")

    @patch('src.dataaug_multi_both.data.dataset._build_redsm5_from_local')
    def test_load_dataset_overlapping_splits(self, mock_build):
        ...
        with pytest.raises(DatasetLoadError, match="Splits are not disjoint"):
            load_hf_dataset("redsm5", validate_disjoint=True)

    @patch('src.dataaug_multi_both.data.dataset._build_redsm5_from_local')
    def test_load_dataset_metadata_includes_stats(self, mock_build):
        """Test that metadata includes dataset statistics."""
        mock_dataset = DatasetDict({
            "train": Dataset.from_list([{"post_id": "1", "text": "a"}, {"post_id": "2", "text": "b"}, {"post_id": "3", "text": "c"}]),
            "validation": Dataset.from_list([{"post_id": "4", "text": "d"}]),
            "test": Dataset.from_list([{"post_id": "5", "text": "e"}, {"post_id": "6", "text": "f"}])
        })
        mock_build.return_value = mock_dataset
