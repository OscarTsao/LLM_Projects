...
### Weights & Biases (Online Tracking)

Set the `wandb.enabled` flag (or `WANDB_ENABLED=true`) plus your API key to push runs to W&B. The trainer logs configuration, training/validation curves, test metrics, and the best checkpoint artifact when `wandb.log_artifacts` is true.

```bash
export WANDB_API_KEY="paste-your-key"
export WANDB_PROJECT="criteria-evidence-agent"
export WANDB_ENABLED=true
PYTHONPATH=. python src/train.py
```

You can customise the `wandb` block in `configs/config.yaml` (project, entity, tags, mode). During Optuna sweeps, each trial launches its own W&B run grouped under the study name.
