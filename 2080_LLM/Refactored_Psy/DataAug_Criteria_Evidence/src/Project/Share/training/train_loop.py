...
class Trainer:
    ...
    def __init__(
        ...
        if use_amp and torch.cuda.is_available():
            ...
            self.amp_context = lambda: autocast(dtype=dtype)
        else:
            self.amp_context = lambda: torch.no_grad().__enter__() or torch.no_grad().__exit__(None, None, None) or DummyContext()
...
class DummyContext:
    """Dummy context manager for non-AMP training."""
    def __enter__(self):
        return self
