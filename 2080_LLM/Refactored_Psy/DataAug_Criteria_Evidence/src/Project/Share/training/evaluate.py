...
class Evaluator:
    ...
    def evaluate(
        ...
        if self.task_type == "criteria":
            metrics = self._compute_criteria_metrics(
                labels, predictions, probabilities, class_names
            )
    ...
    def _compute_criteria_metrics(
        ...
        if class_names and len(class_names) == len(per_class_f1):
            metrics["per_criterion_f1"] = {
                name: float(f1) for name, f1 in zip(class_names, per_class_f1)
            }
        else:
            metrics["per_criterion_f1"] = {
                f"class_{i}": float(f1) for i, f1 in enumerate(per_class_f1)
            }
    ...
    def _compute_evidence_metrics(
        ...
        # F1 at sentence level (both micro and macro)
        metrics["f1_macro"] = f1_score(labels, predictions, average="macro", zero_division=0)
        metrics["f1_micro"] = f1_score(labels, predictions, average="micro", zero_division=0)
        metrics["f1_weighted"] = f1_score(labels, predictions, average="weighted", zero_division=0)
        ...
        if len(np.unique(labels)) == 2:
            ...
            both_wrong = ((labels == 1) & (predictions == 0)) | ((labels == 0) & (predictions == 1))
