...
def configure_mlflow(
    ...
    if config:
        ...
        if hasattr(config, "task") and hasattr(config.task, "task_name"):
            run_tags["task"] = config.task.task_name
        
        if hasattr(config, "model") and hasattr(config.model, "_target_"):
            run_tags["model"] = config.model._target_.split(".")[-1]
