...
def run_train_eval_once(
    data_config: Path = Path("configs/data/redsm5.yaml"),
    pipeline_config: Path = Path("configs/pipeline/default.yaml"),
    evidence_config: Path = Path("configs/evidence/pairclf.yaml"),
    suggest_config: Path = Path("configs/suggest/voi.yaml"),
    criteria_config: Path = Path("configs/criteria/aggregator.yaml"),
    calibration_path: Path = Path("artifacts/calibration.json"),
    seed: int = 42,
    trial: Optional[Any] = None,
) -> float:
    """Run a single pipeline iteration and return a validation metric.

    This helper maintains backward compatibility while allowing HPO integrations
    to call into the pipeline and obtain a scalar metric. Currently it returns
    evidence_macro_f1_present from the evaluation metrics.
    """
    # Reuse main logic but capture the resulting metrics
    ...
    evaluation_agent = EvaluationAgent(args.calibration_path)
    metrics_paths = evaluation_agent.evaluate(predictions, criteria_results, eval_dataset, evaluation_dir)
    # Load metrics and return scalar
    with Path(metrics_paths["val_metrics"]).open("r", encoding="utf-8") as fh:
        metrics = json.load(fh)
    return float(metrics.get("evidence_macro_f1_present", 0.0))
