...
from .data import CriteriaDataset, DatasetSplit, assemble_dataset, create_cross_validation_splits, split_dataset
...
def _train_single(
    ...
    if splits is None:
        ...
        splits = split_dataset(assembled, dataset_cfg)
...
def train(cfg: DictConfig) -> Dict[str, object]:
    """Hydra-managed training entry point supporting optional cross-validation."""
    dataset_cfg = cfg.dataset
    model_cfg = cfg.model
    cross_val_cfg = dataset_cfg.get("cross_validation", {})
    cross_val_enabled = bool(cross_val_cfg.get("enabled", False))
...
def evaluate_checkpoint(cfg: DictConfig, checkpoint: Optional[Path] = None) -> Dict[str, float]:
    ...
    if cross_val_enabled:
        ...
        fold_index = None
        selected_split = split_dataset(assembled, dataset_cfg)
