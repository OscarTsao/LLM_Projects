...
- **Metrics latency (≤100 ms p95)**: Integration test `tests/integration/test_metrics_latency.py` will execute a dry-run training loop that emits ≥200 metric points via the MLflow client while a high-resolution timer captures the elapsed time between `mlflow.log_metric` invocation and completion. Results are aggregated per run, producing p50/p95/p99 latency values. The test fails if p95 > 100 ms. The logging stack MUST run with sanitization enabled to ensure timing reflects production conditions.
- **Checkpoint pruning overhead (≤10 s/save)**: Integration/perf test `tests/integration/test_checkpoint_pruning_overhead.py` will simulate repeated `CheckpointManager.save_checkpoint` calls against synthetic 1–5 GB payloads while the disk monitor reports available space. Each save records start/stop timestamps and the number/size of pruned artifacts. The test reports mean and max pruning duration; it fails if any single save (including aggressive pruning) exceeds 10 s. The test also captures reclaimed-bytes-per-second to support regression tracking.
- Both tests publish their raw timing samples to `outputs/perf/` (git-ignored) for manual inspection and are wired into the CI `slow` marker so they can be executed nightly without impacting developer fast loops.
