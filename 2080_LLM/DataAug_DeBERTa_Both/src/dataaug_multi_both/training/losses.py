...
class DualAgentLoss(nn.Module):
    """Loss function for dual-agent criteria classification.

    This loss only computes the criteria matching loss since evidence embeddings
    are used internally to improve criteria classification (not as a separate task).
    """
    ...
    def __init__(
        ...
        """Initialize dual-agent loss.

        Args:
            criteria_loss_type: Type of criteria loss (bce, focal, weighted_bce, hybrid)
        ...
        if criteria_loss_type == "bce":
            ...
            self.criteria_loss_fn = FocalLoss(gamma=focal_gamma)
        elif criteria_loss_type == "weighted_bce":
            self.criteria_loss_fn = WeightedBCELoss(label_smoothing=label_smoothing)
        elif criteria_loss_type == "hybrid":
            self.criteria_loss_fn = HybridLoss(
    ...
    def forward(
        ...
        """
        # Criteria matching loss
        loss = self.criteria_loss_fn(criteria_logits, criteria_labels.float())
        return loss


# Keep backward compatibility alias
class MultiTaskLoss(DualAgentLoss):
    """Backward compatibility alias for DualAgentLoss."""

    def __init__(
        self,
        criteria_loss_weight: float = 1.0,
        evidence_loss_weight: float = 0.0,  # No longer used
        criteria_loss_type: str = "bce",
        label_smoothing: float = 0.0,
        focal_gamma: float = 2.0
    ):
        """Initialize multi-task loss (backward compatibility).

        Note: evidence_loss_weight is ignored as we no longer have span extraction.
        """
        super().__init__(
        ...
        if evidence_loss_weight != 0.0:
            logger.warning(
                "evidence_loss_weight is deprecated and will be ignored. "
                "Evidence embeddings are now used internally for criteria classification."
            )
