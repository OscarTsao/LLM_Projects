...
This guide walks through setting up and running the storage-optimized evidence-only NLP training pipeline with hyperparameter optimization (HPO) on the RedSM5 mental health dataset.

**Key Capabilities**:
- Evidence span extraction (single-agent)
...
# MLflow Tracking
mlflow_experiment_name: mental_health_evidence
...
**Using MLflow UI**:
1. Navigate to experiment `mental_health_evidence`
2. Sort runs by `val_span_f1` descending
...
experiment = mlflow.get_experiment_by_name("mental_health_evidence")

runs = mlflow.search_runs(
    experiment_ids=[experiment.experiment_id],
    order_by=["metrics.val_span_f1 DESC"],
    max_results=10
)

print("Top 10 Trials:")
print(runs[["params.model_id", "metrics.val_span_f1", "metrics.evidence_span_f1"]])
```
...
- Test set metrics (evidence binding only)
...
df = pd.DataFrame([
    {
        ...
        "evidence_span_f1": r["test_metrics"]["evidence_binding"]["span_f1"],
    }
    for r in reports
])

# Analyze: Which model architectures performed best?
print(df.groupby("model_id")["evidence_span_f1"].agg(["mean", "std", "max"]))
```
...
# TODO: Instantiate your evidence-only model class using config
# model = EvidenceModel.from_config(config)
